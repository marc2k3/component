...
let properties = {
	colour_mode : new Property("2K3.METER.COLOURS.MODE", 1), // 0 UI, 1 custom
	bar_mode : new Property("2K3.METER.BAR.MODE", 0), // 0 rainbow, 1 solid, 2 gradient
	custom_background : new Property("2K3.METER.BACKGROUND.COLOUR", RGB(30, 30, 30)),
	custom_bar : new Property("2K3.METER.BAR.COLOUR", RGB(200, 200, 200)),
	custom_bar_g1 : new Property("2K3.METER.BAR.G1.COLOUR", RGB(0, 128, 255)),
	custom_bar_g2 : new Property("2K3.METER.BAR.G2.COLOUR", RGB(255, 50, 10)),
	custom_peak : new Property("2K3.METER.PEAK.COLOUR"), // no default
	custom_text : new Property("2K3.METER.TEXT.COLOUR", RGB(240, 240, 240)),
	meter_style : new Property("2K3.METER.STYLE", 1), // 0: smooth, 1: blocks
	rms_block_db : new Property("2K3.METER.BLOCK.DB", 0.625),
	rms_3db : new Property("2K3.METER.AES", false),
};

let font_t = CreateFontString("Segoe UI", 8);
let solid_colour = false;
let rms_block_dbs = [0.3125, 0.625, 1.25, 2.5];
let RMS_levels = [], Peak_levels = [], Peak_falldown = [];
let ChannelNames = [ "FL", "FR", "FC", "LFE", "BL", "BR", "FCL", "FCR", "BC", "SL", "SR", "TC", "TFL", "TFC", "TFR", "TBL", "TBC", "TBR" ];
let ww = 0, wh = 0, timer_id = 0, rms_db_offset = 0, dBrange = 0;
let text_layouts = {};
...
function update_colours() {
	if (properties.colour_mode.value == 0) { // UI
		colours.text = window.GetUIColour(ColourType.text);
		colours.highlight = window.GetUIColour(ColourType.highlight);
		colours.background = window.GetUIColour(ColourType.background);
...
function update_graph() {
	let chunk = fb.GetAudioChunk(rms_window);
	...
	if (data && channels.count > 0 && frame_len > 0) {
		let old_count = Peak_levels.length;
		...
		if (old_count < channels.count) {
			for (let c = old_count; c < channels.count; ++c) {
				Peak_levels[c] = 0;
				Peak_falldown[c] = 0;
			}
		}
		...
		for (let c = 0; c < channels.count; ++c) {
			...
			for (let i = c; i < data.length; i += channels.count) {
...
function get_initial_track_info() {
	...
	let info = handle.GetFileInfo();

	let idx = info.InfoFind("channels");
...
function channel_name(ch) {
	if (ch < ChannelNames.length) {
		if (channels.config) {
			for (let i = 0, idx = 0; i < ChannelNames.length; ++i) {
...
function on_mouse_rbtn_up(x, y) {
	...
	if (properties.meter_style.value == 1) {
		...
		let rms_block_db_index = rms_block_dbs.indexOf(properties.rms_block_db.value);
	...
	case 2:
		...
		break;
	case 3:
		colour = utils.ColourPicker(properties.custom_background.value);

		if (colour != properties.custom_background.value) {
			properties.custom_background.value = colour;
			update_colours();
			window.Repaint();
		}
		break;
	case 4:
		colour = utils.ColourPicker(properties.custom_text.value);

		if (colour != properties.custom_text.value) {
			properties.custom_text.value = colour;
			update_colours();
			window.Repaint();
		}
		break;
	case 5:
		colour = utils.ColourPicker(colours.peak);

		if (colour != colours.peak) {
			properties.custom_peak.value = colour;
			update_colours();
			window.Repaint();
		}
		break;
	...
	case 102:
		...
		break;
	case 110:
		colour = utils.ColourPicker(properties.custom_bar.value);

		if (colour != properties.custom_bar.value) {
			properties.custom_bar.value = colour;
			update_colours();
			window.Repaint();
		}
		break;
	case 111:
		colour = utils.ColourPicker(properties.custom_bar_g1.value);

		if (colour != properties.custom_bar_g1.value) {
			properties.custom_bar_g1.value = colour;
			update_colours();
			window.Repaint();
		}
		break;
	case 112:
		colour = utils.ColourPicker(properties.custom_bar_g2.value);

		if (colour != properties.custom_bar_g2.value) {
			properties.custom_bar_g2.value = colour;
			update_colours();
			window.Repaint();
		}
		break;
...
function on_paint(gr) {
	...
	let show_ch_labels = true;
	let show_db_labels = true;
	let bar_pad_left = Scale(20);
	let bar_pad_right = Scale(20);
	let bar_pad_top = Scale(4);
	let bar_pad_bottom = Scale(24);
	let bar_width = ww - bar_pad_left - bar_pad_right;
	let bar_height = Math.floor((wh - bar_pad_top - bar_pad_bottom) / channels.count);
	...
	if (show_db_labels) {
		...
		let y = bar_pad_top + (bar_height * channels.count) + Scale(4);
		gr.FillRectangle(bar_pad_left, y, bar_width, 1, colours.text);

		for (let i = minDB, j = 0; i <= maxDB; i += db_spacing, j++) {
			let name = i + "dB";
			if (!text_layouts[name]) {
				text_layouts[name] = utils.CreateTextLayout(name, font_t, 0, 2);
			}

			let x = bar_pad_left + (bar_width * j / (dBrange / db_spacing));
	...
	if (properties.meter_style.value == 1) { // block mode
		block_count = Math.max(Math.floor(dBrange / properties.rms_block_db.value), 1);
	...
	for (let c = 0; c < channels.count; ++c) {
		if (RMS_levels[c]) {
			let rms_db = Clamp(to_db(RMS_levels[c]) + rms_db_offset, minDB, maxDB);

			if (properties.meter_style.value == 0) { // smooth mode
				let width = Math.round(bar_width * (rms_db - minDB) / dBrange);
				gr.FillRectangle(bar_pad_left, bar_pad_top + (bar_height * c), width, bar_height - 1, colours.bar);
			} else { // block mode
				let blocks = Math.round(block_count * (rms_db - minDB) / dBrange);
		...
		if (peak_bar_width > 0 && Peak_levels[c] > 0) {
			let peak_db = Clamp(to_db(Peak_levels[c]), minDB, maxDB);

			if (peak_db > minDB) {
				let peak_pos = Math.round(bar_width * (peak_db - minDB) / dBrange);
