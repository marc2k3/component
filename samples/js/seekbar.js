function _seekbar(x, y, w, h, spectrogram_mode) {
	...
	this.move = function (x, y) {
		...
		if (this.containsXY(x, y)) {
			if (playback.IsPlaying && playback.Length > 0) {
				...
				_tt(utils.FormatDuration(playback.Length * this.drag_seek));
				if (this.drag) {
					this.playback_seek();
				}
			}
	...
	if (this.spectrogram_mode) {
		...
		this.get_image = function (metadb) {
			...
			this.filename += '.webp';
			return utils.LoadBitmap(this.filename)
		}
		...
		this.rbtn_up_done = function (idx) {
			...
			case 1000:
				...
				if (tmp != this.properties.params.value) {
					...
					this.playback_new_track(playback.GetNowPlaying());
				}
				break;
			case 1001:
		...
		this.run_cmd_async_done = function (task_id) {
			if (this.task_id == task_id) {
				this.working = false;
				this.image = this.get_image(playback.GetNowPlaying());
				window.Repaint();
			}
		}
		...
		this.properties = {
			params : new Property('2K3.SPECTROGRAM.FFMPEG.PARAMS', 's=1024x128'),
			marker_colour : new Property('2K3.SPECTROGRAM.MARKER.COLOUR', RGB(240, 240, 240)),
			library_only : new Property('2K3.SPECTROGRAM.LIBRARY.ONLY', true),
		};
