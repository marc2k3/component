...
function oHeaderBar() {
	...
	this.draw = function (gr) {
		...
		if (this.visible) {
			this.drawColumns(gr);
			if (this.borderDragged && this.borderDraggedId >= 0) {
				// all borders
				for (var b = 0; b < this.borders.length; b++) {
					var lg_x = this.borders[b].x - 2;
					var lg_w = this.borders[b].w;
					var segment_h = 5;
					var gap_h = 5;
					if (b == this.borderDraggedId) {
						...
						if (this.columnDragged == 1) {
							...
							if (this.columnDraggedId > 0 || (this.columnDraggedId == 0 && this.columns[this.columnDraggedId].isHover(x, y))) {
								...
								if (this.columns[this.columnDraggedId].sortOrder) {
									plman.SortByFormat(g_active_playlist, this.columns[this.columnDraggedId].sortOrder, this.sortedColumnDirection);
								} else {
									plman.SortByFormat(g_active_playlist, this.columns[this.columnDraggedId].tf, this.sortedColumnDirection);
								}
								update_playlist();
							} else {
								this.columns[this.columnDraggedId].drag = false;
