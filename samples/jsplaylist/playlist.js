...
function oItem(row_index, type, metadb, track_index, group_index, track_index_in_group, heightInRow, groupRowDelta, obj) {
	...
	this.draw = function (gr, x, y, w, h) {
		...
		if (this.type == 0) { // track
			if (playback.IsPlaying && plman.PlayingPlaylist == g_active_playlist && this.track_index == p.list.nowplaying.PlaylistItemIndex) {
			...
			var lg1_right_field_w = this.r1.calc_width(g_font_group1) + 20;
			var lg2_right_field_w = this.r2.calc_width(g_font_group2) + 20;
...
function oList(object_name) {
	...
	this.init_groups = function () {
		...
		if (properties.showgroupheaders) {
			...
			group_keys = tf_group_key.EvalWithMetadbs(this.handleList);
		}
