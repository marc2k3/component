...
window.DefineScript('Track Info + Seekbar + Buttons', {author:'marc2003'});
...
buttons.update = function () {
	...
	this.buttons.pbo = new Button(panel.w - LM - (bs * 10), y - 2, bs + 4, bs + 4, { char : pbo_chars[pbo], colour: pbo == 0 ? setAlpha(colours.buttons, 60) : colours.contrast }, null, function () { pbo >= pbo_chars.length - 1 ? playback.Order = 0 : playback.Order++ }, 'Playback Order: ' + pbo_names[pbo]);
	this.buttons.stop = new Button(panel.w - LM - (bs * 8), y, bs, bs, { char : chars.stop, colour:playback.StopAfterCurrent ? colours.contrast : colours.buttons}, null, function () { playback.Stop(); }, 'Stop');
	this.buttons.previous = new Button(panel.w - LM - (bs * 7), y, bs, bs, { char : chars.prev, colour:colours.buttons }, null, function () { playback.Previous(); }, 'Previous');
	this.buttons.play = new Button(panel.w - LM - (bs * 6), y, bs, bs, { char : !playback.IsPlaying || playback.IsPaused ? chars.play : chars.pause, colour:colours.buttons}, null, function () { playback.PlayOrPause(); }, !playback.IsPlaying || playback.IsPaused ? 'Play' : 'Pause');
	this.buttons.next = new Button(panel.w - LM - (bs * 5), y, bs, bs, { char : chars.next, colour:colours.buttons }, null, function () { playback.Next(); }, 'Next');
	this.buttons.console = new Button(panel.w - LM - (bs * 3), y, bs, bs, {char : chars.list, colour:colours.buttons }, null, function () { fb.ShowConsole(); }, 'Console');
	this.buttons.search = new Button(panel.w - LM - (bs * 2), y, bs, bs, { char : chars.search, colour:colours.buttons }, null, function () { fb.RunMainMenuCommand('Library/Search'); }, 'Library Search');
	this.buttons.preferences = new Button(panel.w - LM - bs, y, bs, bs, { char : chars.preferences, colour:colours.buttons}, null, function () { fb.ShowPreferences(); }, 'Preferences');
}
...
function on_paint(gr) {
	...
	if (playback.IsPlaying) {
		...
		gr.WriteText(tfo.playback_time.Eval(), normal_font, colours.text, seekbar.x - Scale(72), 0, Scale(60), panel.h, DWRITE_TEXT_ALIGNMENT_TRAILING, DWRITE_PARAGRAPH_ALIGNMENT_CENTER);
		gr.WriteText(tfo.length.Eval(), normal_font, colours.text, seekbar.x + seekbar.w + Scale(12), 0, Scale(60), panel.h, DWRITE_TEXT_ALIGNMENT_LEADING, DWRITE_PARAGRAPH_ALIGNMENT_CENTER);
...
function on_size() {
	panel.size();
	seekbar.x = Scale(300);
	seekbar.w = panel.w - seekbar.x - Scale(310);
	seekbar.h = Scale(6);
	seekbar.y = (panel.h - seekbar.h + Scale(2)) / 2;
	buttons.update();
}
